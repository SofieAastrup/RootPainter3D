...
def get_annot_duration_s(events, fname):
    print("get_annot_duration_s entered")
...
def events_from_client_log(client_log_fpath):
    ...
    for l in lines:
        ...
        if parts[2] == 'update_file_start':
            fname = parts[3].replace('fname:', '').replace('.nii', '')
        event_time = parts[1]
        event_name = parts[2]
        # print("what is going on?: ")    
        # print([p for p in parts if 'fname:' in p])   
        # print([p for p in parts if 'fname:' in p][0])   
        ...
        # print(f"{fname=}: {event_name}  ({event_time})")
