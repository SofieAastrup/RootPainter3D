...
def load_annot_events(proj_dir):
    ...
    if os.path.isfile(client_log_fpath):
        ...
        annot_events = None
    print(f"{len(annot_events)=}")
    return annot_events


def compute_seg_metrics(seg_dir, annot_dir, fname, model_dir, annot_events=None):
    # print(f"compute_seg_metrics, seg_dir={seg_dir}, annot_dir_{annot_dir}, fname={fname}, model_dir={model_dir}")
    # annot and seg are both GZ
    fname = os.path.splitext(fname)[0] + '.gz'
    seg_path = os.path.join(seg_dir, fname)
    if not os.path.isfile(seg_path):
        print(f"{seg_path} is not a file")
    ...
    # seg = imread(seg_path)
    seg = im_utils.load_seg(seg_path)

    if os.path.isfile(annot_path):
        # annot = imread(annot_path)
        annot = im_utils.load_annot(annot_path)
        print("read file with imread")
    else:
        print("no file/no corrections")
        a1, a2, a3 = seg.shape
        annot = np.zeros((2, a1, a2, a3)) # no file implies empty annotation (no corrections)
    
    print(f"annot: {annot.shape}, seg: {seg.shape}")
...
class MetricsPlot:
    ...
    def __init__(self):
        print("initialized MetricsPlot")
        self.plot_window = None
        self.proj_file_path = None
        self.proj_dir = None

    def add_file_metrics(self, fname):
        print("MetricsPlot: add_file_metrics")
    ...
    def view_plot_from_csv(self, csv_fpath):
        print("MetricsPlot: view_plot_from_csv")
        # CSV Headers.
    ...
    def create_metrics_plot(self, proj_file_path, navigate_to_file, selected_fpath): 
        print("MetricsPlot: create_metrics_plot")
